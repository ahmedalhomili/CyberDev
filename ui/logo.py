import random
import sys
import time

from ui.colors import RESET, COLOR_SCHEMES, BOLD


def get_gradient_line(text, colors):
    """تطبيق تدرج لوني على سطر واحد"""
    result = ""
    color_count = len(colors)
    text_len = len(text)

    for i, char in enumerate(text):
        color_index = int((i / text_len) * color_count) % color_count
        result += colors[color_index] + char

    return result + RESET


def logo(animated=False):
    """عرض اللوجو بألوان متداخلة تتغير في كل مرة"""

    # اختيار مجموعة ألوان عشوائية
    colors = random.choice(COLOR_SCHEMES)

    # خلط الألوان للتنوع
    if random.choice([True, False]):
        colors = colors[::-1]  # عكس الترتيب أحياناً

    logo_lines = [
        "   ██████╗ ██╗   ██╗██████╗ ███████╗██████╗     ██████╗   ███████╗ ██╗   ██╗",
        "  ██╔════╝ ╚██╗ ██╔╝██╔══██╗██╔════╝██╔══██╗    ██╔═══██╗ ██╔════╝ ██║   ██║",
        "  ██║       ╚████╔╝ ██████╔╝█████╗  ██████╔╝    ██║   ██║ █████╗   ██║   ██║",
        "  ██║        ╚██╔╝  ██╔══██╗██╔══╝  ██╔══██╗    ██║   ██║ ██╔══╝   ╚██╗ ██╔╝",
        "  ╚██████╗    ██║   ██████╔╝███████╗██║  ██║██╗ ╚█████╔╝  ███████╗  ╚████╔╝ ",
        "   ╚═════╝    ╚═╝   ╚═════╝ ╚══════╝╚═╝  ╚═╝╚═╝  ╚════╝   ╚══════╝   ╚═══╝  ",
    ]

    subtitle_color = random.choice(colors)
    accent_color = random.choice([c for c in colors if c != subtitle_color])

    print("\n")

    for i, line in enumerate(logo_lines):
        # تدرج لوني مختلف لكل سطر
        shifted_colors = colors[i % len(colors):] + colors[:i % len(colors)]
        colored_line = get_gradient_line(line, shifted_colors)

        if animated:
            print(colored_line)
            time.sleep(0.1)
        else:
            print(colored_line)

    print()

    # العنوان الفرعي
    subtitle1 = "              Safe Web Vulnerability Checker"
    subtitle2 = "                    Passive • Ethical"

    print(f"{subtitle_color}{BOLD}{subtitle1}{RESET}")
    print(f"{accent_color}{subtitle2}{RESET}")

    # خط فاصل
    separator = "    " + "═" * 50
    print(f"{colors[0]}{separator}{RESET}")
    print()


def logo_animated():
    logo(animated=True)


def logo_matrix():
    """لوجو بستايل ماتريكس مع تأثير الكتابة"""
    matrix_green = '\033[38;5;46m'
    dark_green = '\033[38;5;22m'

    logo_lines = [
        "   ██████╗ ██╗   ██╗██████╗ ███████╗██████╗     ██████╗ ███████╗██╗   ██╗",
        "  ██╔════╝ ╚██╗ ██╔╝██╔══██╗██╔════╝██╔══██╗    ██╔══██╗██╔════╝██║   ██║",
        "  ██║       ╚████╔╝ ██████╔╝█████╗  ██████╔╝    ██║  ██║█████╗  ██║   ██║",
        "  ██║        ╚██╔╝  ██╔══██╗██╔══╝  ██╔══██╗    ██║  ██║██╔══╝  ╚██╗ ██╔╝",
        "  ╚██████╗    ██║   ██████╔╝███████╗██║  ██║██╗ ╚█████╔╝███████╗ ╚████╔╝ ",
        "   ╚═════╝    ╚═╝   ╚═════╝ ╚══════╝╚═╝  ╚═╝╚═╝  ╚════╝ ╚══════╝  ╚═══╝  ",
    ]

    print("\n")
    for line in logo_lines:
        for char in line:
            if char == '█':
                sys.stdout.write(f"{matrix_green}{char}")
            elif char in '╔╗╚╝║═':
                sys.stdout.write(f"{dark_green}{char}")
            else:
                sys.stdout.write(f"{matrix_green}{char}")
            sys.stdout.flush()
            time.sleep(0.002)
        print(RESET)

    print()
    print(f"{matrix_green}{BOLD}              Safe Web Vulnerability Checker{RESET}")
    print(f"{dark_green}                    Passive • Ethical{RESET}")
    print()


if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("  🎨 عرض اللوجو العادي (ألوان مختلفة كل مرة)")
    print("=" * 60)

    for i in range(3):
        print(f"\n--- الاستدعاء رقم {i + 1} ---")
        logo()
        time.sleep(1)

    print("\n" + "=" * 60)
    print("  💚 لوجو ماتريكس (مع تأثير الكتابة)")
    print("=" * 60)
    logo_matrix()
