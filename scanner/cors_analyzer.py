...
class CORSAnalyzer:
    ...
    def check_allow_origin(self) -> Optional[Finding]:
        ...
        if acao == '*':
            ...
            if acac == 'true':
                return Finding(
                    title='CRITICAL: Wildcard CORS with Credentials',
                    severity='HIGH',
                    description='Access-Control-Allow-Origin is set to * while '
                               'Access-Control-Allow-Credentials is true. '
                               'This allows ANY origin to access resources with user credentials.',
                ...
                return Finding(
                    ...
                    description='Access-Control-Allow-Origin: * allows any origin to access resources. '
                               'While not exploitable without credentials, this is overly permissive.',
    ...
    def check_allow_methods(self) -> Optional[Finding]:
        ...
        if any(m in methods for m in dangerous_methods):
            ...
            if acao == '*':
                return Finding(
                    title='Dangerous CORS Methods with Wildcard Origin',
                    severity='HIGH',
                    description=f'Wildcard CORS combined with methods: {", ".join(methods)}. '
                               'Any origin can modify/delete resources.',
                    location='CORS Headers',
